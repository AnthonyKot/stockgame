...
- Later on 2026-09-10 (Claude): the same block authored for the other nine cases by Sonnet writers under cases/SNAPSHOT_PRESENTATION_BRIEF.md, validated and merged by scripts/merge_presentation.py; label guard added to verify_cases.py; money() formats trillions for the bank. Codex review items 1-5 done. Then a scene review of the nine non-Nektar cases (research/scene-review/README.md): 38 pre-decision text fixes in scenes.json, market-data findings rejected as false positives, compound-assumption rewording deferred to the user.
