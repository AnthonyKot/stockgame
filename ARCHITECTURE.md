...
| `IDEAS.md` | Three candidate directions side by side with effort, chances and kill criteria, the shared answer-unit task, the recommended branch order (idea3 → idea2 → idea1). |
| `idea1/` | The campaign: `EPIC.md` (scenario, twelve stories, open decisions) and `README.md` (product-owner assessment). |
| `idea2/` | Investigate by asking: `EPIC.md` (search first, grounded assistant only on evidence; defines `cases/<id>/units.json`) and `README.md`. |
| `idea3/` | The case as a text quest: `EPIC.md` (quest graphs over the same units, plain-language decision screen) and `README.md`. Recommended first branch. |
...
| `scripts/build_campaign.py` | research/selected.json, research/prices/{ticker,SPY,BTC-USD}.json | site/data/campaigns/<id>/manifest.json + prices/<instrument>/<year>.json | Campaign manifest (idea 1): the five long/skip conventions, SPY bar dates as the session calendar, horizons resolved to actual sessions and dropped when beyond coverage, per-year price slices un-adjusted for later splits, Bitcoin UTC daily closes. Fails on an audit problem. |
...
- Campaign (idea 1 prototype `proto-3`: Nektar, Madrigal, JPMorgan): `site/campaign-engine.js` is a pure ledger (frozen dollar budgets and reservations, fractional fills at the open, ex-date cash dividends with the simulator's entitlement rule, idempotent events, replay, report with the cash-plus-Bitcoin baseline); `site/campaign-store.js` persists one run under `stockgame.campaign.v1` with read-back verification and a stale-tab check; `site/campaign.html` shows the portfolio strip, a since-your-last-stop summary, the story embedded from `play.html?mode=campaign` (no ticket, no journal, no reveal), the campaign ticket with the dollar budget and funding check, advance-to-next-stop, closure debriefs via `STORY.atExit` at the exit date (reveal and outcome files fetched only then), and the final report. `node scripts/test_campaign.js` runs the engine plan's fixture and the real campaign end to end against `SIM.simulate` and the ledger identity; `scripts/test_campaign_ui.cjs` is the browser journey. Data is bounded by the clock: `build_campaign.py` cuts price slices at every possible stop (`prices/<instrument>/upto-<date>.json`; the last session before each cutoff and every offered exit; Bitcoin at the day before each cutoff and every exit) and precomputes `debrief/<scene>/<exit>.json` with `scripts/slice_debrief.js` (site/story.js at the exit date, plus the reveal), so the page never requests a bar beyond its clock or an outcome file; the browser test asserts both. An all-skip run finishes at the last decision. Static files remain reachable by URL; that limit is stated in the manifest.
