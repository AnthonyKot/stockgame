...
| `scripts/merge_dated_debrief.py` | cases/<id>/dated_debrief.json (writer output) | cases/scenes.json | Validates status words, word limits, assumption and event ids, then folds each file into `scenes[<id>].dated_debrief` and deletes it. `--check-only` validates without merging. |
...
- Exit-scoped feedback: `node scripts/test_dated_debrief.js` (after a build) checks every case: baseline covers all assumptions, updates cite real events, and each available 1/3/5-year exit with public events gets an applicable update.
...
- Remaining: user playtest; broader opening/investigation wording and editorial readiness triage. No campaign integration in this patch.
- 2026-09-10 (Claude): rolled `dated_debrief` out to the other nine cases with Sonnet writers under `cases/DATED_DEBRIEF_BRIEF.md`, merged by `scripts/merge_dated_debrief.py`, checked by `scripts/test_dated_debrief.js` (10 cases, 28 horizon exits).
